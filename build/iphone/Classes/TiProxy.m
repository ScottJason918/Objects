...
//Rare exceptions to indicate a bug in the _objects code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_objects = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_objects]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.objects.module"
